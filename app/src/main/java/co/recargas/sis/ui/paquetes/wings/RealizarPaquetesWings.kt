...
class RealizarPaquetesWings : AppCompatActivity(), DetallesPaquete {
   ...
   // private var recargaRepository: RecargaRepository = RecargaRepository(application)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when(tipo){
            "wings"->{
                var internet=ProductoFragmentWings()
    ...
    inner class EnviarPaquete:AsyncTask<Void,Int,Boolean>(){
        ...
        override fun onPostExecute(result: Boolean?) {
            ...
            if(result==true){
                Toast.makeText(this@RealizarPaquetesWings,"Recarga Exitosa ${saldo}", Toast.LENGTH_SHORT).show()
                val builder = AlertDialog.Builder(this@RealizarPaquetesWings)
                ...
                Toast.makeText(this@RealizarPaquetesWings, "Recargar fallida ${respuesta}",Toast.LENGTH_SHORT).show()
