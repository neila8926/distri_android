...
class RealizarPaquetesKalley : AppCompatActivity(), DetallesPaquete {
   ...
   // private var recargaRepository: RecargaRepository = RecargaRepository(application)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when(tipo){
            "kalley"->{
                var internet=ProductoFragmentKalley()
    ...
    inner class EnviarPaquete:AsyncTask<Void,Int,Boolean>(){
        ...
        override fun onPostExecute(result: Boolean?) {
            ...
            if(result==true){
                Toast.makeText(this@RealizarPaquetesKalley,"Recarga Exitosa ${saldo}", Toast.LENGTH_SHORT).show()
                val builder = AlertDialog.Builder(this@RealizarPaquetesKalley)
