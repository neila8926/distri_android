...
class RealizarPaquetesExito : AppCompatActivity(), DetallesPaquete {
   ...
   // private var recargaRepository: RecargaRepository = RecargaRepository(application)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when(tipo){
            "exito"->{
                var internet=ProductoFragmentExito()
    ...
    inner class EnviarPaquete:AsyncTask<Void,Int,Boolean>(){
        ...
        override fun onPostExecute(result: Boolean?) {
            ...
            if(result==true){
                Toast.makeText(this@RealizarPaquetesExito,"Recarga Exitosa ${saldo}", Toast.LENGTH_SHORT).show()
                val builder = AlertDialog.Builder(this@RealizarPaquetesExito)
